...
pipeline {
    ...
    environment {
        ...
        E2E_SUITES = 'blackbox'
        TEST_RESULT_XML = 'results/test-result-blackbox.xml'
        TEST_RESULT_XMLS = 'results/test-result-blackbox.xml'
    ...
    stages {
        ...
        stage('test-blackbox') {
            ...
            post {
                always {
                    junit allowEmptyResults: true, skipMarkingBuildUnstable: true, testResults: "${env.PROJECT_DIR}/results/test-result-blackbox.xml"
                }
            }
        }
    }
