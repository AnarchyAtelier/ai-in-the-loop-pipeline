...
pipeline {
    ...
    environment {
        ...
        E2E_SUITES = 'naive'
        TEST_RESULT_XML = 'results/test-result-naive.xml'
        TEST_RESULT_XMLS = 'results/test-result-naive.xml'
    ...
    stages {
        ...
        stage('test-naive') {
            ...
            post {
                always {
                    junit allowEmptyResults: true, skipMarkingBuildUnstable: true, testResults: "${env.PROJECT_DIR}/results/test-result-naive.xml"
                }
            }
        }
    }
